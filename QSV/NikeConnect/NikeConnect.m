...
-(id) init{
    ...
    _NikeConnectActivity = [[NSMutableArray alloc] init];    return self;
}
    ...
    if (![Nike isLoggedIn]){
        ...
        [Nike logInWithCompletion:^(BOOL success, id json, NSError *error){
            ...
            if (![[NSUserDefaults standardUserDefaults] objectForKey:@"userName"]) {
             ...
             //   [alertView show];
                [[NSUserDefaults standardUserDefaults] setObject:[NSString stringWithFormat:@"Nike"] forKey:@"userName"];
...
-(void)request10Days{
    [self requestDataOfSize:@10 Offset:@1];
    NSLog(@"reuqest10Days returning %@", _NikeConnectActivity);
}
-(void)request100Days{
    [self requestDataOfSize:@100 Offset:@1];
}
-(NSMutableArray*)getNikeActivity{
    return _NikeConnectActivity;
}
...
-(void)monthlyRequest:(NSNumber*) offset{
    [self requestDataOfSize:@30 Offset:[NSNumber numberWithInt:[offset integerValue]*30+1]];
}
-(BOOL)isLoading{
    return _loading;
}

- (void)requestDataOfSize:(NSNumber*)count Offset:(NSNumber*)offset{
    _loading = TRUE;
    ...
    [Nike performRequestWithHTTPMethod:NikeHTTPMethodGET path:@"/me/sport/activities" parameters:params bodyData:nil completion:^(BOOL success, id json, NSError *error) {
        if (success){
            _loading = FALSE;
           // NSLog(@"JSON: %@",json);
            NSMutableArray* thisArray = [[NSMutableArray alloc] initWithArray:[json objectForKey:@"data"]];
            [_NikeConnectActivity addObject:[[NSMutableArray alloc] initWithArray:[json objectForKey:@"data"]]];
            for(NSDictionary *element in thisArray){
                if([[element objectForKey:@"deviceType"] isEqualToString:@"FUELBAND"]){
                   /* if([self createDatabaseEntryWithActivityId:[element objectForKey:@"activityId"]
                                                          Date:[element objectForKey:@"startTime"]
                                                       Summary:[element objectForKey:@"metricSummary"]])
                        NSLog(@"added to the database: %@", [element objectForKey:@"startTime"]);
                        [self requestMetrics:[element objectForKey:@"activityId"] Date:[element objectForKey:@"startTime"]];
                  */
                }
            }
