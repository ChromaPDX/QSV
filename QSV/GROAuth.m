...
+ (void)loginWithGoodreadsWithCallbackURL:(NSString *)callbackURL completion:(void( ^ )(NSDictionary *authParams, NSError *error))completion {
    ...
    static OAuth1Controller *thisGROController;
    
    // If the user doesn't submit a callback URL, just use the default one
    // The way GROController works, it really doesn't matter what the URL is since
    ...
    thisGROController = [[OAuth1Controller alloc] init];
                      
    // Set consumerKey and consumerSecret to the values set earlier
    thisGROController.consumerKey = [GROAuth consumerKey];
    thisGROController.consumerSecret = [GROAuth consumerSecret];
    thisGROController.oauthCallback = callbackURL;
    
    // Now use the webView to log the user in
    [thisGROController loginWithWebView:loginWebView
...
+ (NSURLRequest *)goodreadsRequestForOAuthPath:(NSString *)path
    ...
    OAuth1Controller *thisGROController = [[OAuth1Controller alloc] init];
    
    // Set consumerKey and consumerSecret to the values set earlier
    thisGROController.consumerKey = [GROAuth consumerKey];
    thisGROController.consumerSecret = [GROAuth consumerSecret];
    
    NSURLRequest *request = [thisGROController preparedRequestForPath:path parameters:parameters HTTPmethod:method oauthToken:oauth_token oauthSecret:oauth_token_secret];
    return request;
}
...
+ (NSString *)XMLResponseForNonOAuthPath:(NSString *)path parameters:(NSDictionary *)parameters {
    ...
    path = [NSString stringWithFormat:@"%@%@?", GR_API_URL,path];
