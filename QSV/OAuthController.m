...
/*
#define OAUTH_CALLBACK       @"http://www.chroma.io" //Sometimes this has to be the same as the registered app callback url
#define CONSUMER_KEY         @"375d7f484cf34330b93c084dfd378f38"
#define CONSUMER_SECRET      @"7b13e6a08cf6443884fe0ef38f214ca5"
...
static NSDictionary* _oAuthParams;
#pragma mark -
...
+ (void)setOAuthParams:(NSDictionary *)oAuthParams{
    NSLog(@"setting oAuthParams...");
    _oAuthParams = [NSDictionary dictionaryWithDictionary:oAuthParams];
    NSLog(@"in setOAuthParams AUTH_URL = %@", _oAuthParams[@"AUTH_URL"]);
...
// URL: https://www.goodreads.com/oauth/authorize?oauth_token=yAH2DAuvt5Aa72RX06EPRQ&oauth_callback=http%3A%2F%2Fwww.goodreads.com
// URL: https://www.goodreads.com/oauth/authorize?oauth_token=LEcJFJqt1VxgJuOV2RdKng&oauth_callback=http%3A%2F%2Fwww.goodreads.com
...
{
    NSLog(@"AUTH_URL = %@", _oAuthParams[@"AUTH_URL"]);
    NSString *request_url = [_oAuthParams[@"AUTH_URL"] stringByAppendingString:_oAuthParams[@"REQUEST_TOKEN_URL"]];
    NSString *oauth_consumer_secret = _oAuthParams[@"CONSUMER_SECRET"];
    
    NSLog(@"request_url = %@", request_url);
    NSMutableDictionary *allParameters = [self.class standardOauthParameters];
    if ([_oAuthParams[@"OAUTH_SCOPE_PARAM"] length] > 0) [allParameters setValue:_oAuthParams[@"OAUTH_SCOPE_PARAM"] forKey:@"scope"];

    NSString *parametersString = CHQueryStringFromParametersWithEncoding(allParameters, NSUTF8StringEncoding);
    
    NSString *baseString = [_oAuthParams[@"REQUEST_TOKEN_METHOD"] stringByAppendingFormat:@"&%@&%@", request_url.utf8AndURLEncode, parametersString.utf8AndURLEncode];
    ...
    request.HTTPMethod = _oAuthParams[@"REQUEST_TOKEN_METHOD"];
    ...
    NSLog(@"in obtainRequestTokenWithCompletion, request = %@", request);
...
{
    NSString *oauth_callback = _oAuthParams[@"OAUTH_CALLBACK"];
    NSString *authenticate_url = [_oAuthParams[@"AUTH_URL"] stringByAppendingString:_oAuthParams[@"AUTHENTICATE_URL"]];
   ...
   // authenticate_url = [authenticate_url stringByAppendingFormat:@"&display=touch"];
    authenticate_url = [authenticate_url stringByAppendingFormat:@"?mobile=1"];
    ...
    NSLog(@"in authenticateToken, request = %@", request);

    _delegateHandler = ^(NSDictionary *oauthParams) {
        if (oauthParams[@"oauth_verifier"] == nil || [oauthParams[@"authorize"] isEqualToString:@"0"]) {
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
    if (_delegateHandler) {
        ...
        if ([urlWithoutQueryString rangeOfString:_oAuthParams[@"OAUTH_CALLBACK"]].location != NSNotFound)
        {
...
{
    NSString *access_url = [_oAuthParams[@"AUTH_URL"] stringByAppendingString:_oAuthParams[@"ACCESS_TOKEN_URL"]];
    NSString *oauth_consumer_secret = _oAuthParams[@"CONSUMER_SECRET"];
    ...
    NSString *baseString = [_oAuthParams[@"ACCESS_TOKEN_METHOD"] stringByAppendingFormat:@"&%@&%@", access_url.utf8AndURLEncode, parametersString.utf8AndURLEncode];
    ...
    request.HTTPMethod = _oAuthParams[@"ACCESS_TOKEN_METHOD"];
...
+ (NSMutableDictionary *)standardOauthParameters
{
    ...
    NSString *oauth_consumer_key = _oAuthParams[@"CONSUMER_KEY"];
                             ...
                             oauthSecret:(NSString *)oauth_token_secret
{
    ...
    NSString *request_url = _oAuthParams[@"API_URL"];
